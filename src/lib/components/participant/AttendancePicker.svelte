<script lang="ts">
		...
		switch (type) {
			case 'rehearsal':
				if (laxInclude(participant, concertOrRehearsal)) {
					participant.rehearsals.splice(
						participant.rehearsals.indexOf(concertOrRehearsal as Rehearsal),
						1
					);
				} else {
			...
			case 'concert':
				if (laxInclude(participant, concertOrRehearsal)) {
					participant.concerts.splice(
						participant.concerts.indexOf(concertOrRehearsal as Concert),
						1
					);
				} else {
	...
	// Fonction corrigée pour vérifier l'inscription du participant
	function laxInclude(
		participant: CustomParticipant | Participant,
		concertOrRehearsal: Concert | Rehearsal
	) {
		// Si le concert/répétition a une liste de participants, on vérifie dedans (logique de la page attendance)
		if (concertOrRehearsal.participants) {
			return concertOrRehearsal.participants.some((p) => p.id === participant.id);
		}

		// Sinon, on utilise l'ancienne logique (pour les cas où on n'a pas la liste complète)
		return (
			participant.rehearsals!.includes(concertOrRehearsal as Rehearsal) ||
			participant.concerts!.includes(concertOrRehearsal as Concert)
		);
	}
</script>

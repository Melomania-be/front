...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
	...
	<div
		class="bg-[#E7E7E7] rounded-[10px] shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden"
	>
		...
		<div class="flex-1 overflow-y-auto p-4 space-y-4">
			...
			<div class="bg-white border-2 border-[#8C8C8C] rounded-[10px] p-6">
				<div
					class="border-2 border-dashed rounded-[8px] p-8 text-center transition-colors {dragActive
				...
				>
					...
					<h3 class="text-gray-700 mb-4 text-lg font-bold">DRAG AND DROP FILES HERE</h3>
			...
			{#if selectedFiles.length > 0}
				<div class="bg-white border-2 border-[#8C8C8C] rounded-[10px] p-4">
					...
					<div
						class="max-h-{isMobile ? '60' : '48'} overflow-y-auto space-y-{isMobile ? '2' : '3'}"
					>
						{#each selectedFiles as file, index}
							<div
								class="flex items-center justify-between p-{isMobile
									? '3'
									: '4'} bg-gradient-to-br from-white to-gray-50 border-2 border-gray-300 rounded-[10px] hover:border-[#6B9AD9] transition-all duration-200 group"
							>
								<div class="flex items-center gap-{isMobile ? '2' : '3'} flex-1 min-w-0">
									<!-- ✅ DESIGN IDENTIQUE : File icon même style -->
									<div
										class="w-12 h-12 bg-gray-100 rounded-[8px] border-2 border-gray-300 group-hover:border-[#6B9AD9] transition-colors flex items-center justify-center"
									>
									...
									<div class="flex-1 min-w-0">
										...
										>
											<h3
												class="font-bold text-gray-900 truncate {isMobile ? 'text-sm' : ''}"
												title={file.name}
											>
												{file.name}
											</h3>
											{#if !isMobile}
												<span
													class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded font-semibold border border-blue-300"
												>
										...
										<div
											class="grid grid-cols-1 {isMobile ? 'gap-1' : 'md:grid-cols-2 gap-4'} text-sm"
										>
											<div class="min-w-0">
								...
								<button
									class="p-2 text-gray-600 hover:text-red-600 rounded-lg hover:bg-red-50 border border-transparent hover:border-red-300 transition-colors {isMobile
										? 'opacity-100'

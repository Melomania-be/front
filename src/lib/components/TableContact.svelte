			...
			const filteredContacts = data.data.filter((contact) => {
				...
				return contact.instruments.some((instrument) => {
					return selectedLevelInstruments.some(([id, level]) => {
						return instrument.id === id && instrument.pivot_proficiency_level == level;
					});
				});
			});
								...
								>
									...
									{#if options.orderBy === column}
										{#if options.order === 'asc'}
											<Fa
												icon={faSort}
												class="text-[16px] -right-4 absolute"
												style="color: #b6b6b6;"
											/>
											<Fa
												icon={faSortUp}
								...
								{#if row.projects.length !== 0}
									<div class="grid-cols-3 grid gap-2">
										{#each row.projects as project}
											<div
												class="flex gap-4 my-1 text-xs min-h-10 pl-2 rounded-xl border-2 border-gray-400 relative"
											>
												<div class="font-semibold self-center mr-8">{project.name}</div>
												<div
													class="text-white font-bold w-7 text-center bg-blue-300 ml-auto p-1 px-2 rounded-br-[11px] rounded-tl-xl absolute right-[-1px] bottom-[-1px]"
												>
													{project.id}
												</div>
											</div>

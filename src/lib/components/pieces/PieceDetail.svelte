<script lang="ts">
	...
	import { Music, Download, Upload, Plus, FileText, Eye, Folder } from 'lucide-svelte';
...
<div class="bg-white border-2 border-[#8C8C8C] rounded-[10px] p-6">
	...
	<div class="border-t-2 border-[#E7E7E7] pt-6">
		<div class="flex items-center gap-2 mb-4">
			<Folder class="text-green-500" size={20} />
			<h3 class="font-bold text-lg text-gray-700">SCORES</h3>
		</div>
		...
		{:else if scores.length === 0}
			<div class="text-center py-8">
				...
				<p class="text-gray-500 mb-2">No scores available for this piece</p>
				<button
					class="text-[#6B9AD9] hover:underline"
					on:click={() => showUploader = true}
				>
					Upload the first score
				</button>
			</div>
		{:else}
	...
	<div class="border-t-2 border-[#E7E7E7] pt-6 mt-6">
		<h3 class="font-bold text-lg text-gray-700 mb-4">PROJECTS USING THIS PIECE</h3>
				...
				{#each piece.sections as project}
					<div class="p-4 bg-gradient-to-r from-[#6CB1C8] to-[#5077BA] text-white rounded-lg">
						...
						<p class="text-sm opacity-80">
							{project.pieces?.length || 0} piece{project.pieces?.length !== 1 ? 's' : ''} total
						</p>
					</div>
				{/each}
			</div>
		{:else}
			<div class="text-center py-4">
				<p class="text-gray-500">This piece is not used in any projects yet</p>
			</div>
		{/if}
	</div>
</div>

...
<script lang="ts">
	...
	function saveCustomStatuses() {
		localStorage.setItem(
			`recruitment_custom_statuses_${projectId}`,
	...
	function applyFilters() {
		...
		if (filters.source) {
			filtered = filtered.filter(
				(c) => c.source && c.source.toLowerCase().includes(filters.source.toLowerCase())
			);
		}
		if (filters.section) {
			filtered = filtered.filter((c) =>
				c.section?.name.toLowerCase().includes(filters.section.toLowerCase())
			);
		}
		if (filters.contacted_by) {
			filtered = filtered.filter(
				(c) =>
					c.contacted_by &&
					c.contacted_by.toLowerCase().includes(filters.contacted_by.toLowerCase())
			);
		}
	...
	async function fetchContacts() {
		...
		try {
			const response = await fetch(
				`/api/projects/${projectId}/management/recruitment${optionInUrls}`
			);
			...
			if (response.ok) {
				...
				const safeContacts = Array.isArray(data.data)
					? data.data.map((contact) => ({
							...
							display_name:
								`${contact.first_name || 'First name'} ${contact.last_name || 'Last name'}`.trim(),
	...
	async function updateContactStatus(contactId: number, updateData: any) {
		try {
			const response = await fetch(
				`/api/projects/${projectId}/management/recruitment/contacts/${contactId}`,
				{
					method: 'PUT',
					...
					body: JSON.stringify({
						...updateData,
						contact_date:
							updateData.status === 'awaiting_response' ? new Date().toISOString() : undefined
					})
				}
			);
	...
	async function sendBulkEmails() {
		...
		const emailContacts = contacts.filter(
			(c) => c && selectedContacts.includes(c.id) && c.email && c.email.trim().length > 0
		);
		...
		try {
			const response = await fetch(
				`/api/projects/${projectId}/management/recruitment/send-emails`,
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
			...
			if (response.ok) {
				...
				if (result.success) {
					...
					alert(
						`Emails processed: ${result.sent?.length || 0}, Failed: ${result.failed?.length || 0}`
					);
				}
	...
	async function sendRecommendationEmail(contact: RecruitmentContact) {
		...
		try {
			const response = await fetch(
				`/api/projects/${projectId}/management/recruitment/send-recommendation-email`,
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
	...
	function shouldHighlightFollowUp(contact: RecruitmentContact): boolean {
		if (!contact || contact.status !== 'awaiting_response' || !contact.contact_date || !settings)
			return false;
	...
	function getSourceDisplay(contact: RecruitmentContact): string {
		...
		if (
			...
			contact.recommended_by.trim() !== '' &&
			contact.recommended_by !== 'Anonymous'
		) {
		...
		return contact.source === 'database'
			? 'Database'
	...
	function isRecommendedContact(contact: RecruitmentContact): boolean {
		return (
			contact.source === 'recommendation' ||
			(contact.recommended_by && contact.recommended_by.includes('Recommended by '))
		);
	}
	...
	{#if showFilters}
		<div class="bg-gray-50 border-2 border-gray-200 rounded-[10px] p-4 mb-4">
			<div class="flex items-center justify-between mb-3">
				...
				<button on:click={clearFilters} class="text-sm text-blue-600 hover:text-blue-800 underline">
					Clear all
				</button>
			</div>
				...
				<div class="flex flex-wrap gap-2">
					...
					{#each allStatuses as status}
						<button
							...
							class="px-3 py-2 text-sm rounded-[6px] font-semibold {getStatusColor(
								status
	...
	{#if safeContacts.length === 0 && !isRefreshing}
		<div class="text-center py-12">
			...
			<p class="mt-1 text-sm text-gray-500">
				Start by adding contacts manually or importing from the database.
			</p>
		</div>
	{:else}
		<SimpleFilterer
			...
			{#if isMobile}
				...
				<div class="space-y-3">
					{#each safeContacts as contact (contact.id)}
						<div
							class="border-2 border-[#8C8C8C] rounded-[10px] p-4 hover:bg-gray-50 {shouldHighlightFollowUp(
								contact
							)
								? 'bg-yellow-50'
								: ''} {contact.is_duplicate ? 'bg-orange-50' : ''}"
						>
							...
							<div class="flex items-start justify-between mb-3">
								...
								<div class="relative">
									<select
										...
										class="text-xs px-2 py-1 border border-gray-300 rounded {getStatusColor(
											contact.status
										)} font-medium"
									>
										{#each allStatuses as status}
							...
							<div class="mb-3">
								<div class="bg-white border-2 border-[#8C8C8C] rounded-[8px] p-3">
									<div class="flex items-center space-x-3 mb-3">
										<div
											class="flex items-center justify-center w-8 h-8 bg-[#6B9AD9] rounded-[6px]"
										>
									...
									<div class="grid grid-cols-1 gap-3">
										<div class="bg-gray-50 border-2 border-gray-300 rounded-[6px] p-3">
											<div class="flex items-center mb-2">
												<svg
													class="w-3 h-3 text-gray-500 mr-2"
													fill="none"
													stroke="currentColor"
													viewBox="0 0 24 24"
												>
								...
								<div class="mb-3 p-3 bg-blue-50 border-2 border-blue-200 rounded-[8px]">
									<div class="flex items-center">
										...
										<span class="text-sm font-bold text-blue-700"
											>Contacted by: {contact.contacted_by}</span
										>
									</div>
								</div>
							{/if}
				...
				<div class="w-full overflow-x-auto">
					<table class="w-full min-w-[1200px] text-sm text-left text-gray-500">
							...
							<tr>
								<th class="px-4 py-3">
									<input
										...
										checked={selectedContacts.length === safeContacts.length &&
								...
								<th
									class="px-4 py-3 cursor-pointer hover:bg-gray-200 transition-colors"
						...
						<tbody>
							{#each safeContacts as contact (contact.id)}
								<tr
									class="border-b hover:bg-gray-50 {shouldHighlightFollowUp(contact)
										? 'bg-yellow-50'
										: ''} {contact.is_duplicate ? 'bg-orange-50' : ''}"
								>
									<td class="px-4 py-3">
									...
									<td class="px-4 py-3">
										<div class="space-y-1">
											<div class="inline-block">
												<span
													class="text-xs px-2 py-1 rounded-[4px] border font-bold {getSourceStyle(
														contact
													)}"
												>
													{#if getSourceIcon(contact)}
									...
									<td class="px-4 py-3">
										<div class="bg-white border border-[#8C8C8C] rounded-[6px] p-2">
											<div class="flex items-center space-x-2 mb-2">
												<div
													class="flex items-center justify-center w-6 h-6 bg-[#6B9AD9] rounded-[4px]"
												>
													<svg
			...
			<div class="p-6">
				...
				<div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
					...
					<p class="text-xs text-blue-700">
						Not yet contacted, Awaiting response, Follow up, Not available, Pending validation,
						Cancelled, Recruited
					</p>
				</div>
			</div>
	...
	<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
		<div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
			<div class="flex items-center justify-between p-6 border-b">
				...
				<button on:click={closeRecommendationEmailModal} class="text-gray-400 hover:text-gray-600">
					<X size={20} />
				</button>
			</div>
			...
			<div class="p-6">
				<p class="text-sm text-gray-600 mb-4">
					Send a special recommendation email to <strong
						>{selectedRecommendationContact.first_name}
						{selectedRecommendationContact.last_name}</strong
					>
				...
				{#if selectedRecommendationContact.recommended_by}
					<div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
						<p class="text-sm text-blue-800">
							...
							{getSourceDisplay(selectedRecommendationContact)}
						</p>
					</div>
				{/if}
			</div>

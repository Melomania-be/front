...
<script lang="ts">
	...
	async function importSelectedContacts() {
		...
		try {
			...
			if (response.ok) {
				...
				if (
					importResults &&
					(importResults.imported.length > 0 || importResults.conflicts.length > 0)
				) {
	...
	function getContactInstruments(contact: Contact): string {
		...
		return contact.instruments
			.map((i) => `${i.name} (${i.pivot_proficiency_level || 'Not specified'})`)
			.join(', ');
	}
										...
										<thead class="bg-gray-50 sticky top-0">
											<tr>
												<th class="px-4 py-3 text-left w-12">
													<input
														type="checkbox"
														checked={selectedContacts.length ===
															data.data.filter((c) => !isContactInProject(c)).length &&
															data.data.filter((c) => !isContactInProject(c)).length > 0}
														on:change={(e) =>
															e.target.checked ? selectAllContacts() : clearSelection()}
														class="rounded"
													/>
												</th>
												...
												<tr class="border-t hover:bg-gray-50 {isInProject ? 'bg-yellow-50' : ''}">
													...
													<td class="px-4 py-3">
														<div class="flex items-center space-x-3">
															<div
																class="w-10 h-10 bg-[#6B9AD9] rounded-full flex items-center justify-center"
															>
																<span class="text-white font-bold text-sm">
																	{(contact.firstName || contact.first_name || 'F').charAt(0)}{(
																		contact.lastName ||
																		contact.last_name ||
																		'L'
																	).charAt(0)}
															...
															<div>
																<div class="font-medium text-gray-900">
																	{contact.firstName || contact.first_name || 'First name'}
																	{contact.lastName || contact.last_name || 'Last name'}
																</div>
													...
													<td class="px-4 py-3">
														<span
															class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {contact.validated
			...
			<div class="p-6">
				<div class="mb-4">
					...
					<div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
						<p class="font-semibold">
							{duplicateContact?.firstName || duplicateContact?.first_name}
							{duplicateContact?.lastName || duplicateContact?.last_name}
						</p>
				...
				<div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
					<p class="text-sm text-red-700">
						<strong>Warning:</strong> This contact appears to be similar to existing contacts in your
						recruitment list. Importing duplicates may cause confusion in your recruitment process.
					</p>
				</div>

				<p class="text-gray-600 mb-6">Do you want to import this contact anyway?</p>
			</div>

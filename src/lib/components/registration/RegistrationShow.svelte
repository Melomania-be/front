<script lang="ts">
	...
	// Sorted content blocks avec intégration du filtre showOnRegistration
	$: aboveContents = (registration.contents || [])
		.filter(c => c.position === 'above' && c.showOnRegistration)
		.sort((a, b) => (a.order ?? 0) - (b.order ?? 0));

	$: belowContents = (registration.contents || [])
		.filter(c => (!c.position || c.position === 'below') && c.showOnRegistration)
		.sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
</script>
...
{#if showPopupSubmit}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
		<div class="bg-white p-6 rounded-xl shadow-xl {!isMobile ? 'w-[60%] text-lg' : 'w-[90%] text-[15px]'} text-center text-gray-600">
			...
			<p>All communication will be sent from <span class="text-blue-400">noreply@melomania.be</span></p>
		...
		<div class="h-auto pb-6 flex justify-center">
			<div class="bg-white w-[80%] rounded-xl h-auto registration-content">
				<div class="form-head h-auto">
					...
					{#if step === 0}
						<div class="tabs-details">
							<button class:active={tabName === 'event'} on:click={() => (tabName = 'event')}>Events</button>
							<button class:active={tabName === 'program'} on:click={() => (tabName = 'program')}>Program</button>
							<button class:active={tabName === 'info'} on:click={() => (tabName = 'info')}>Information</button>
						</div>
					{/if}
				</div>
				<div class="from-body rounded-xl">
                     ...
                     downloadCalendarEvent({
                      ...
                      description: event.comment
									...
									{#if isMobile}
										{#if combinedEvents.length > 0}
											{#each combinedEvents as event}
												<div class="bg-white border-[2px] border-[#989898] pl-[10px] m-[15px] p-[5px] rounded-[12px] text-xs relative">
													...
													{#if event.type === 'rehearsal'}
														<div class="absolute font-semibold bottom-[-0.5px] right-[-0.5px] bg-[#6B9AD9] text-white p-2 rounded-tl-2xl rounded-br-[10px]">
															{event.type}
														</div>
													{:else}
														<div class="absolute font-semibold bottom-[-0.5px] right-[-0.5px] bg-[#a584d2] text-white p-2 rounded-tl-2xl rounded-br-[10px]">
															{event.type}
														</div>
													{/if}
												</div>
								...
								{#if tabName === 'program'}
									...
									{:else if registration.project?.pieces}
										{#each registration.project.pieces as piece}
											<div class="bg-white border-2 border-gray-400 m-2 rounded-2xl text-[13px] flex pl-4 p-2 items-center">
									...
									{#if registration.contents && registration.contents.length > 0}
										<div class="bg-[#ececec] h-full pl-7 pr-7 overflow-y-auto pb-7">
											...
											{#each aboveContents as content}
												<h2 class="text-2xl font-medium tracking-tight text-black dark:text-white mb-3 mt-5">
													{@html content.title}
												</h2>
												<div class="w-full flex">
					...
					<!-- Step 2 : Contact -->
					{#if step === 1}
						<div class="overflow-x-hidden ml-6 w-full self-center pt-5 pb-5 h-full mr-6 flex flex-col gap-3 content-contact">
							<div class="flex flex-col h-16">
								<div class="-mb-2 text-[12px] bg-white z-20 ml-6 font-semibold pl-3 pr-3 text-gray-500 {contactErrors.first_name ? 'text-red-500 placeholder-red-400' : ''}" style="width: fit-content;">
									First Name*
								</div>
								<input
									class="p-3 border-2 border-gray-500 rounded-xl focus:outline-none {contactErrors.first_name ? 'border-red-500 placeholder-red-400' : ''}"
									bind:value={newContact.first_name}
									placeholder="First name"
									required
								/>
								{#if contactErrors.first_name}
									<div class="text-red-500 text-right text-xs ml-10 mr-2 mt-1">First name is required</div>
								{/if}
							</div>

							<div class="flex flex-col h-16">
								<div class="-mb-2 text-[12px] bg-white z-20 ml-6 font-semibold pl-3 pr-3 text-gray-500 {contactErrors.last_name ? 'text-red-500 placeholder-red-400' : ''}" style="width: fit-content;">
									Last Name*
								</div>
								<input
									class="p-3 border-2 border-gray-500 rounded-xl focus:outline-none {contactErrors.last_name ? 'border-red-500 placeholder-red-400' : ''}"
									bind:value={newContact.last_name}
									placeholder="Last name"
									required
								/>
								{#if contactErrors.last_name}
									<div class="text-red-500 text-right text-xs ml-10 mr-2 mt-1">Last name is required</div>
								{/if}
							</div>

							<div class="flex flex-col h-16">
								<div class="-mb-2 text-[12px] bg-white z-20 ml-6 font-semibold pl-3 pr-3 text-gray-500 {contactErrors.email || contactErrors.validEmail ? 'text-red-500 placeholder-red-400' : ''}" style="width: fit-content;">
									Email Adress*
								</div>
								<input
									class="p-3 border-2 border-gray-500 rounded-xl focus:outline-none {contactErrors.email || contactErrors.validEmail ? 'border-red-500 placeholder-red-400' : ''}"
									bind:value={newContact.email}
									placeholder="example@email.com"
									required
								/>
								{#if contactErrors.email}
									<div class="text-red-500 text-right text-xs ml-10 mr-2 mt-1">Email is required</div>
								{/if}
								{#if contactErrors.validEmail}
									<div class="text-red-500 text-right text-xs ml-10 mr-2 mt-1">Invalid email address</div>
								{/if}
							</div>

							<div class="flex flex-col h-16">
								<div class="-mb-2 text-[12px] bg-white z-20 ml-6 font-semibold pl-3 pr-3 text-gray-500" style="width: fit-content;">
									Phone Number
								</div>
								<input
							...
							{#if registration.project?.sectionGroup?.sections}
								<div class="flex flex-col h-16">
									<div class="-mb-2 text-[12px] bg-white z-20 ml-6 font-semibold pl-3 pr-3 text-gray-500 {contactErrors.section_id ? 'text-red-500 placeholder-red-400' : ''}" style="width: fit-content;">
										Section*
									</div>
									<select
										class="p-3 border-2 border-gray-500 rounded-xl focus:outline-none {contactErrors.section_id ? 'border-red-500 placeholder-red-400' : ''}"
									...
									{#if newContact.section_id === 0}
										<div class="-mt-[37px] bg-white z-20 ml-3 text-gray-400" style="width: fit-content;">
											Section*
										</div>
									{/if}
									{#if contactErrors.section_id}
										<div class="text-red-500 text-right text-xs ml-10 mr-2 mt-4">Section is required</div>
									{/if}
								</div>
							{/if}
						</div>
           ...
           if (validateContactFields()) {
            ...
            step = step + 1;
					...
					{#if step === 2}
						<div class="{!isMobile ? 'h-[50vh]' : 'h-[60vh]'} bg-[#ececec] pt-5 w-full pb-10 form-body-content">
							<div class="mx-3">
								<h3 class="text-xl font-bold tracking-tight text-gray-500 dark:text-white mb-2">Attendances</h3>
								...
								{#if isMobile}
									{#if combinedEvents.length > 0}
										{#each combinedEvents as event}
											<div class="bg-white border-[2px] border-[#989898] pl-[10px] m-[15px] p-[5px] rounded-[12px] text-xs relative flex flex-col">
							...
							{#if registration.form && registration.form.length > 0}
								<div class="mt-10 ml-6">
									<h3 class="text-xl font-bold tracking-tight text-gray-500 dark:text-white mb-6">Additional informations</h3>

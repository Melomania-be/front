...
function truncatePlace(place: string, maxLength: number = 8): string {
    ...
    if (place.length > 4 && maxLength > 5) {
        ...
        const consonants = remaining.split('').filter(char =>
          !vowels.includes(char.toLowerCase()) && /[a-zA-Z]/.test(char)
        );
...
function addHeader(doc: jsPDF, project: Project, pageNumber: number = 1, showTitle: boolean = false): number {
    if (showTitle) {
...
function createSingleCompactTable(
  ...
  pageNumber: number = 1
    ...
    doc.line(startX + nameColWidth + sectionColWidth, currentY, startX + nameColWidth + sectionColWidth, currentY + 14);
    ...
    events.forEach(event => {
        ...
        const displayText = dateTimeStr.length > maxChars ?
          dateTimeStr.substring(0, maxChars - 1) + '.' : dateTimeStr;
    ...
    doc.line(startX + nameColWidth + sectionColWidth, currentY, startX + nameColWidth + sectionColWidth, currentY + 10);
    ...
    sectionGroups.forEach((sectionParticipants, sectionName) => {
        ...
        sectionParticipants.forEach(participant => {
            ...
            doc.line(startX + nameColWidth + sectionColWidth, currentY, startX + nameColWidth + sectionColWidth, currentY + rowHeight);
...
function addCompactSummary(doc: jsPDF, data: AttendanceData, startY: number): number {
    ...
    doc.text(`• ${participants.length} participants across ${totalSections} sections`, 25, startY + 18);
    doc.text(`• ${concerts.length} concerts and ${rehearsals.length} rehearsals scheduled`, 25, startY + 25);
...
export async function generateCompactAttendancePDF(data: AttendanceData): Promise<void> {
    ...
    if (concerts.length > 0) {
        currentY = createCompactEventsTable(doc, concerts, participants, project, laxInclude, currentY, 'Concerts', pageNumber);
    }
    ...
    if (rehearsals.length > 0) {
        ...
        currentY = createCompactEventsTable(doc, rehearsals, participants, project, laxInclude, currentY, 'Rehearsals', pageNumber);
    }

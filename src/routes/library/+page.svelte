<script lang="ts">
	...
	async function loadData() {
		try {
			const composersResponse = await fetch(
				'/api/composers?page=1&limit=250&filter=&orderBy=id&order=asc'
			);
			...
			const piecesResponse = await fetch(
				'/api/pieces?page=1&limit=250&filter=&orderBy=id&order=asc'
			);
			...
			const typesResponse = await fetch(
				'/api/type_of_pieces?page=1&limit=250&filter=&orderBy=id&order=asc'
			);
			if (typesResponse.ok) {
	...
	function viewComposer(composer: any) {
		...
		goto(
			`/library/composers?page=1&limit=250&filter=&orderBy=id&order=asc&selected=${encodedComposer}`
		);
	}
	...
	function viewType(type: any) {
		...
		goto(
			`/library/type_of_pieces?page=1&limit=250&filter=&orderBy=id&order=asc&selected=${encodedType}`
		);
	}
</script>
						...
						<table class="w-full">
							<thead class="border-b-2 border-gray-300">
								<tr class="text-left">
									...
									<th class="p-3 font-semibold text-gray-700 uppercase text-sm w-20 text-center"
										>Actions</th
									>
								</tr>
							</thead>
								...
								{#each composers.slice(0, 10) as composer}
									<tr class="border-b border-gray-200 hover:bg-gray-50">
										<td class="p-3 text-gray-900"
											>{composer.longName || composer.shortName || '-'}</td
										>
										<td class="p-3 text-gray-600">{composer.country || '-'}</td>
										<td class="p-3 text-gray-600"
											>{composer.birthDate ? new Date(composer.birthDate).getFullYear() : '-'}</td
										>
							...
							<tbody>
								{#each pieces.slice(0, 10) as piece}
									<tr class="border-b border-gray-200 hover:bg-gray-50">
										...
										<td class="p-3 text-gray-600"
											>{piece.composer?.longName || piece.composer?.shortName || '-'}</td
										>
					...
					<div class="overflow-x-auto">
						<table class="w-full">
							<thead class="border-b-2 border-gray-300">
								<tr class="text-left">
									<th class="p-3 font-semibold text-gray-700 uppercase text-sm">Name</th>
									<th class="p-3 font-semibold text-gray-700 uppercase text-sm text-right"
										>Pieces Count</th
									>
									<th class="p-3 font-semibold text-gray-700 uppercase text-sm w-20 text-center"
										>Actions</th
									...
									<tr>
										<td colspan="3" class="p-3 text-center">
											<a
												...
												class="text-[#6B9AD9] hover:underline font-semibold"
											>
												View {typeOfPieces.length - 10} more categor{typeOfPieces.length - 10 > 1
													? 'ies'
													: 'y'}
											</a>
										</td>

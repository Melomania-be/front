<script lang="ts">
	...
	let selectedCategories: number[] = data.contractor.categories?.map((category:any) => category.id) ?? [];

	async function updateContractor() {
		const response = await fetch(`/api/contractor/${data.contractor.id}`, {
			method: 'PUT',
			headers: {
	...
	async function createCategory() {
		console.log("Create category API call to be implemented:", newCategory);
...
<div class="max-w-3xl mx-auto p-6">
	<h1 class="text-2xl font-bold mb-6">Edit Contractor</h1>

	<div class="bg-white rounded-lg shadow p-4 max-w-xl">
		<div class="mb-4">
			<label class="block mb-1">First Name</label>
			<input class="border p-2 w-full rounded" bind:value={firstName} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Last Name</label>
			<input class="border p-2 w-full rounded" bind:value={lastName} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Email</label>
			<input class="border p-2 w-full rounded" bind:value={email1} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Email 2</label>
			<input class="border p-2 w-full rounded" bind:value={email2} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Email 3</label>
			<input class="border p-2 w-full rounded" bind:value={email3} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Phone 1</label>
			<input class="border p-2 w-full rounded" bind:value={phone1} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Phone 2</label>
			<input class="border p-2 w-full rounded" bind:value={phone2} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Phone 3</label>
			<input class="border p-2 w-full rounded" bind:value={phone3} />
		</div>

		<div class="mb-4">
			<label class="block mb-1">Organization</label>
			<select class="border p-2 w-full rounded" bind:value={organizationId}>

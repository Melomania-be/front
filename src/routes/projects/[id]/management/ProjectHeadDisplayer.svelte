		...
		{#if project}
			<div class="flex items-center p-2">
				...
				>
					<div class="">
						<p>
							Created at: {#if project.createdAt}<DateShow
									startTime={project.createdAt}
								/>{:else}Unknown{/if}
						</p>
						<p>
							Updated at: {#if project.updatedAt}<DateShow
									startTime={project.updatedAt}
								/>{:else}Unknown{/if}
						</p>
					</div>
				</div>
			...
			<!--Tabs-->
			<div class="ml-5 w-[95%] text-lg text-gray-400 font-semibold flex gap-[3vw] overflow-x-auto">
				<button
					class="flex items-center gap-2 p-3 {selectedTab === 0
				...
				>
					<Fa
						...
						class="text-[16px]"
						style="color: {selectedTab === 0 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Project Details
				</button>
				<button
					class="flex gap-2 items-center p-3 {selectedTab === 1
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToParticipants}
				>
					<div class="flex items-center gap-2">
						<Fa
							icon={faUsers}
							class="text-[16px]"
							style="color: {selectedTab === 1 ? '#6B9AD9;' : ' #9ca3af;'}"
						/>
						Participants
				...
				<button
					class="flex items-center gap-2 p-3 {selectedTab === 2
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToMailing}
				>
					<Fa
						icon={faEnvelope}
						class="text-[16px]"
						style="color: {selectedTab === 2 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Mailing
				</button>
				<button
					class="flex gap-2 items-center p-3 {selectedTab === 3
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToCallsheet}
				>
					<Fa
						icon={faSheetPlastic}
						class="text-[16px]"
						style="color: {selectedTab === 3 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Callsheet
				</button>
				<button
					class="flex gap-2 items-center p-3 {selectedTab === 4
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToAttendance}
				>
					<Fa
						icon={faCalendarCheck}
						class="text-[16px]"
						style="color: {selectedTab === 4 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Attendances
				</button>
				<button
					class="flex gap-2 items-center p-3 {selectedTab === 5
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToAuditions}
				>
					<Fa
						icon={faMusic}
						class="text-[16px]"
						style="color: {selectedTab === 5 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Auditions
				</button>
				<button
					class="flex gap-2 items-center p-3 {selectedTab === 6
						? 'text-[#6B9AD9] border-b-[3px] border-[#6B9AD9]'
						: ''}"
					on:click={navigateToAccounting}
				>
					<Fa
						icon={faWallet}
						class="text-[16px]"
						style="color: {selectedTab === 6 ? '#6B9AD9;' : ' #9ca3af;'}"
					/>
					Accounting
				</button>
			</div>
		...
		>
			...
			>
				...
				>
					<div class="">
						<p>
							Created at : {#if project?.createdAt}<DateShow startTime={project.createdAt} />{:else}
								-
							{/if}
						</p>
						<p>
							Updated at : {#if project?.updatedAt}<DateShow startTime={project.updatedAt} />{:else}
								-
							{/if}
						</p>
					</div>

...
{#if popUpStats}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
		<div
			class="bg-white p-6 rounded-xl shadow-xl h-auto max-h-[80%] text-center {isMobile
				? 'w-[90%]'
				: 'w-2/3'}"
		>
			...
			<div class="mt-2">
				...
				<div
					class="grid gap-2 max-h-[40vh] overflow-y-auto p-2 {isMobile
						? 'grid-cols-1'
						: 'grid-cols-3'}"
				>
					{#each safeParticipants as participant}
						{#if participant}
							<div
								class="text-sm border border-gray-400 rounded-full py-1 px-2 {safeParticipantsSeenCallsheet.find(
							...
							>
								{#if project?.id && participant?.id}
									<a href="/projects/{project.id}/management/participants/{participant.id}">
										{participant?.contact?.firstName || 'Unknown'}
										{participant?.contact?.lastName || 'Name'} (Participant profile)
									</a>
								{/if}
			...
			<div class="border-[#8C8C8C] rounded-[10px] border-2 p-4 bg-white w-full">
				<div class="flex mb-4">
					...
					<div class="ml-auto mr-0">
						...
						{:else}
							<span
								class="text-gray-400 font-bold text-sm bg-gray-300 p-2 rounded-[8px] cursor-not-allowed"
								>New callsheet</span
							>
						{/if}
					</div>
					...
					{#if safeCallsheets.length > 0}
						{#each safeCallsheets.reverse() as callsheet}
							{#if callsheet && project?.id}
								<li
									class="border-2 p-2 rounded-full border-gray-300 flex justify-center {maxCallsheet?.id ===
									callsheet.id
										? 'bg-blue-100'
										: ''}"
								>
										...
										<div class="flex items-center space-x-1">
											<a href="/projects/{project.id}/management/callsheets/{callsheet.id}">
												<h3 class="text-sm font-medium text-gray-900 dark:text-white">
													Version : {callsheet.version || 'Unknown'}
													<span class="text-gray-400 mx-2">|</span>
													Last Update : {callsheet.updatedAt
														? new Date(callsheet.updatedAt).toLocaleDateString('en-US', {
																year: 'numeric',
																month: 'long',
																day: 'numeric'
															})
				...
				{#if safeProjectParticipants.length > 0}
					<div class="flex mt-1">
						<div class="w-full bg-gray-200 rounded-full dark:bg-gray-700">
							<div
								...
								style="width: {safeProjectParticipants.length > 0
									? ((safeParticipants.length - safeParticipantsSeenCallsheet.length) /
											safeProjectParticipants.length) *
										100
									: 0}%;"
							></div>
						</div>
					...
					<div>
						{safeParticipants.length - safeParticipantsSeenCallsheet.length} / {safeProjectParticipants.length}
						participants have seen the last callsheet
					</div>
					<button
				...
				<div class="flex items-center">
					...
					{#if project?.id}
						{#if project.registration}
							<a
								class="ml-auto text-white font-bold text-sm hover:bg-blue-700 bg-[#6B9AD9] border-red-400 p-2 rounded-[8px]"
								href="/projects/{project.id}/management/registration">Edit registration form</a
							>
						{:else}
							<a
								class="ml-auto text-white font-bold text-sm hover:bg-blue-700 bg-[#6B9AD9] p-2 rounded-[8px]"
								href="/projects/{project.id}/management/registration">Add registration form</a
							>
						{/if}
					{:else}
						<span
							class="ml-auto text-gray-400 font-bold text-sm bg-gray-300 p-2 rounded-[8px] cursor-not-allowed"
							>Registration</span
						>
					{/if}
				</div>
				...
				{:else}
					<div class="text-gray-500 my-6 mt-4 ml-6">
						<p>
							...
							{project.registration.updatedAt
								? new Date(project.registration.updatedAt).toLocaleDateString('en-US', {
										year: 'numeric',
										month: 'long',
										day: 'numeric'
									})
						...
						<p>
							...
							{project.registration.createdAt
								? new Date(project.registration.createdAt).toLocaleDateString('en-US', {
										year: 'numeric',
										month: 'long',
										day: 'numeric'
									})

			...
			<div class="p-4 rounded-lg dark:bg-gray-800 w-full">
				<ul class="flex flex-col gap-2">
					{#if !isMobile}
						...
						{:else}
							<div class="px-10 text-sm py-4 text-center text-gray-500">No events found</div>
						{/if}
					{/if}
					{#if isMobile}
						{#if eventsToDisplay.length > 0}
							...
							{/each}
						{:else}
							<div class="px-6 text-sm py-4 text-center text-gray-500">No events found</div>
						{/if}
					{/if}
...
{:else}
	<div
		class="w-full h-64 border-[#8C8C8C] rounded-[10px] border-2 bg-white dark:bg-gray-800 dark:border-gray-700 flex justify-center items-center"
	>

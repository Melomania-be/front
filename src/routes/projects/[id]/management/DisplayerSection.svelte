...
{#if popUpSection}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
		<div
			class="bg-white p-6 rounded-xl shadow-xl h-auto items-center flex justify-center flex-col
     {isMobile ? 'w-[90%]' : 'w-1/3'}"
		>
			...
			<div class="h-full w-[95%] mb-4">
				...
				<div class={participantsInSection.length > 1 ? 'grid grid-cols-2' : 'mx-[25%]'}>
					{#each participantsInSection as p}
						<p
							class="text-sm border whitespace-nowrap truncate overflow-hidden border-gray-400 rounded-full text-center my-1 px-2 mx-1 font-semibold text-gray-500 {participantsNotValidated.find(
								(pnv) => pnv?.id === p?.id
							)
								? 'bg-gray-200'
								: 'bg-white'}"
		...
		{#if project?.sectionGroup?.sections && project.sectionGroup.sections.length > 0}
			{#if !isMobile}
				...
				<div class="text-xs gap-2 text-center">
					<div
						class="flex flex-row gap-2 w-full h-[100px] border-t border-t-dotted border-l-2 border-l-[#8C8C8C] px-1"
					>
                ...
                {participantCount >= (sectionSizeMap.get(section) ?? 1)
									? 'bg-[#D1A9FF] hover:bg-purple-400'
									: 'bg-blue-200 hover:bg-blue-300'}"
								style="height: {(participantCount / (sectionSizeMap.get(section) ?? 1)) * 100}%;"
							>
								<div
									class="tooltip font-s pointer-events-none {showParticipantsNumber
										? 'visible'
										: ''}"
								>
				...
				<button
					class="flex items-center gap-1 font-bold text-sm p-2 rounded-[8px] mt-4 {clickedStyle}"
					on:click={() => changeSorting()}
				>
					<Fa
						icon={faArrowDownWideShort}
						class="text-[14px]"
						style="color: {!sort ? 'white' : '#6B9AD9'};"
					/>
					Fill Rate
				</button>
			{/if}
				...
				<div class="text-xs h-auto grid grid-cols-[2fr_6fr_1fr] w-full">
					{#each countArray as [section, participantCount]}
						<div class="flex h-full flex-col w-full border-l-2 border-white px-1 text-right">
							{#if project.sectionGroup?.sections}
								{@const sectionObj = project.sectionGroup.sections.find((s) => s?.id === section)}
								<p class="flex-1 leading-tight text-[9px] pt-[0.1rem]">
									{sectionObj?.name || 'Unknown'}
								</p>
							{/if}
						</div>
			...
			{/if}
		{:else}
			<p class="text-sm text-center mb-4 py-8 text-gray-500">
				No Section Group or sections available
			</p>
		{/if}
	</div>
{:else}
	<div
		class="px-4 py-4 w-full border-[#8C8C8C] rounded-[10px] border-2 bg-white dark:bg-gray-800 dark:border-gray-700 flex justify-center items-center h-64"
	>

...
<script lang="ts">
	...
	function syncAuditionStatuses() {
		...
		participants = participants.map((participant) => {
			const audition = auditionsData.find(
				(a) => a.participant && a.participant.id === participant.id
			);
		...
		if (currentParticipant) {
			...
			if (updatedCurrentParticipant) {
				...
				console.log(
					'🔄 Updated current participant:',
					currentParticipant.contact.firstName,
					'status:',
					currentParticipant.audition_status
				);
			}
		}
	...
	function isParticipantInAudition(participant: Participant | null): boolean {
		...
		console.log(
			'🔍 Checking audition status for participant:',
			participant.contact.firstName,
			participant.contact.lastName
		);
		...
		if (auditionsData && auditionsData.length > 0) {
			...
			if (audition) {
				console.log(
					'✅ Found audition for participant:',
					audition.is_submitted ? 'completed' : 'pending'
				);
				return true;
			}
		}
	...
	async function deleteParticipantWithoutEmail() {
		...
		try {
			...
			if (response.ok) {
				...
				closeDeleteModal();
				showNotification(
					`${participantName} has been deleted successfully (no email sent)`,
					'success'
				);
				...
				if (audition) {
					...
					try {
						const deleteResponse = await fetch(
							`/api/projects/${data.id}/management/auditions/${audition.id}`,
							{
								method: 'DELETE'
							}
						);
	...
	function openAuditionModal() {
		...
		if (isParticipantInAudition(currentParticipant)) {
			...
			alert(
				`This participant is already in audition (status: ${statusText}). Cannot request another audition.`
			);
			console.log(
				'🚫 Audition blocked - participant already in audition:',
				currentParticipant.audition_status
			);
			return;
		}
	...
	async function requestAudition() {
		...
		if (isParticipantInAudition(currentParticipant)) {
			const statusText = getAuditionStatusText(currentParticipant);
			alert(
				`Error: This participant is already in audition (status: ${statusText}). Cannot request another audition.`
			);
			closeAuditionModal();
			return;
		}
	...
	async function manualRefresh() {
		...
		await Promise.all([loadParticipants(), loadAuditionsData(), loadAuditionStats()]);
	}
</script>
				...
				<div class="flex {isMobile ? 'flex-col gap-3' : 'justify-between items-center'}">
					<div>
						...
						<p class="text-sm text-gray-600 mt-1">
							Manage applications, request auditions and track submissions • {auditionStats.submitted}
							submitted • {auditionStats.pending} pending
						</p>
					</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					{#if participants && participants.length > 0}
						<div class="divide-y divide-gray-200 max-h-96 overflow-y-auto">
							{#each participants as participant}
								<div
									class="p-4 hover:bg-gray-50 cursor-pointer transition-colors {currentParticipant?.id ===
									participant.id
										...
										<div class="flex-1">
											...
											{#if isParticipantInAudition(participant)}
												...
												{#if audition}
													{#if audition.is_submitted && audition.submitted_at}
														<span
															class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 mt-1"
														>
															Audition completed
														</span>
													{:else}
														<span
															class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 mt-1"
														>
												...
												{:else}
													...
													{#if badge}
														<span
															class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full {badge.class} mt-1"
														>
															{badge.text}
														</span>
													{/if}
													{#if participant.audition_deadline}
														<p class="text-xs text-gray-500 mt-1">
															Deadline: {new Date(participant.audition_deadline).toLocaleDateString(
																'en-GB'
															)}
														</p>
													{/if}
				...
				<div class="bg-white border-2 border-[#8C8C8C] rounded-[10px]">
					{#if currentParticipant}
						<div class="px-6 py-4 border-b border-gray-200">
							<h2 class="text-xl font-semibold text-gray-900">
								APPLICATION FROM {currentParticipant.contact.firstName.toUpperCase()}
								{currentParticipant.contact.lastName.toUpperCase()}
							</h2>
						</div>
									...
									<div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
										...
										{:else}
											<p class="text-sm text-yellow-600 mt-2">
												⏳ The audition is in progress. The candidate can download PDFs for their
												section and upload their recordings.
											</p>
										{/if}
									</div>
								{:else}
											...
											<div class="flex items-center justify-between">
												<div class="flex items-center">
													...
													{#if currentParticipant.audition_deadline}
														<span class="ml-3 text-sm text-gray-600">
															Deadline: {new Date(
																currentParticipant.audition_deadline
								...
								<div class="bg-gray-50 rounded-lg p-4 space-y-2">
									<div class="grid grid-cols-2 gap-4">
										...
										<div>
											...
											<a
												href="mailto:{currentParticipant.contact.email}"
										...
										<div>
											...
											<p class="text-gray-900">
												{currentParticipant.contact.phone || 'Not provided'}
											</p>
										</div>
										<div>
											<span class="text-sm font-medium text-gray-700">Messenger:</span>
											<p class="text-gray-900">
												{currentParticipant.contact.messenger || 'Not provided'}
											</p>
										</div>
									</div>
							...
							<div class="border-t border-gray-200 pt-6">
									...
									{:else}
										...
										>
											<span class="relative z-10"
												>🎭 Audition {getAuditionStatusText(currentParticipant)}</span
											>
										</button>
									{/if}
								</div>
								...
								<div class="border-t border-gray-100 pt-4">
									...
									<p class="text-xs text-gray-500 mt-2 text-center">
										⚠️ Use "Delete silently" when rejection was already communicated through another
										channel
									</p>
								</div>
					...
					{:else}
						<div class="p-12 text-center">
							...
							<p class="text-gray-600 mt-1">
								Click on an application in the list to view its details
							</p>
						</div>
					{/if}
				</div>
						...
						<div class="mt-2">
							<p class="text-sm text-gray-500">
								Are you sure you want to delete <strong
									>{currentParticipant?.contact.firstName}
									{currentParticipant?.contact.lastName}</strong
								> from this project?
							</p>
						</div>
					</div>
				...
				<div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
					<div class="flex">
						...
						<div class="text-sm text-yellow-700">
							...
							<p class="mt-1">
								This will permanently delete the participant <strong
									>without sending any email notification</strong
								>. Use this only when rejection has already been communicated through another
								channel.
							</p>
						</div>
					</div>
		...
		<div class="bg-white rounded-lg shadow-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
			<div class="px-6 py-4 border-b border-gray-200">
				...
				<p class="text-gray-700 mt-1">
					You are about to request an audition from <strong
						>{currentParticipant?.contact.firstName} {currentParticipant?.contact.lastName}</strong
					>
			...
			<div class="px-6 py-4 space-y-6">
				...
				<div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
					<div class="flex items-center">
						...
						<div class="text-sm text-blue-700">
							...
							<p>
								The candidate will automatically receive all PDFs configured for their section. They
								can download them from their audition portal.
							</p>
						</div>
					</div>
				</div>
				...
				<div>
					...
					<div
						...
						class="bg-white border border-gray-300 rounded-lg min-h-[200px] {isRequestingAudition
					...
					<div class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
						<div class="flex items-center">
							...
							<div class="text-sm text-blue-700">
								...
								<p>
									If you don't set a deadline, it will automatically be set to <strong
										>1 day before the first rehearsal</strong
									> of the project.
								</p>
							</div>
						</div>
				...
				<div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
					<div class="flex items-center">
						...
						<div class="text-sm text-yellow-700">
							...
							<p>
								The candidate can only upload <strong>audio</strong> (MP3, WAV, etc.) or
								<strong>video</strong> (MP4, AVI, MOV, etc.) files.
							</p>
						</div>
					</div>
				</div>
		...
		<div class="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
			...
			<div class="px-6 py-4 space-y-4">
				<p class="text-gray-700">
					You are about to reject the participation of <strong
						>{currentParticipant?.contact.firstName} {currentParticipant?.contact.lastName}</strong
					>.
				</p>
				<p class="text-sm text-gray-600">
					A rejection email will be automatically sent to the participant. You can add a custom
				...
				<div>
					...
					<div
						...
						class="bg-white border border-gray-300 rounded-lg min-h-[200px] {isRefusing
							? 'opacity-50 pointer-events-none'
							: ''}"
						style="font-family: inherit;"
					></div>

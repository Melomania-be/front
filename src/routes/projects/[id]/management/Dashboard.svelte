...
{#if project}
	<div class="h-auto p-4 {isMobile ? 'bg-[#E7E7E7] w-screen' : 'bg-[#E7E7E7]'} border-2">
		...
		<div class="grid {isMobile ? 'grid-cols-1' : ' grid-cols-2'} items-center w-full mb-4">
			<div
				class="bg-white border-2 border-[#E35656] rounded-[10px] {isMobile ? 'w-full' : ' w-[90%]'}"
			>
				<Notification bind:participantsWithoutEmail bind:project bind:participantsNotValidated />
			</div>

			<div
				class="flex text-white h-[100px] font-bold {isMobile
					? 'w-full text-xs mt-4'
					: ' ml-auto mr-4'}"
			>
				<div class="flex w-full text-center {isMobile ? 'gap-3' : 'gap-6'}">
					<div class="rounded-lg py-2 px-4 flex-1 h-full bg-[#6CB1C8]">
						<p>PARTICIPANTS</p>
						<p class="font-extrabold {isMobile ? 'mt-6 mobile-stat-text' : '-mt-1 text-[45px]'}">
		...
		<div
			class="h-full {isMobile
				? 'w-full'
				: 'w-2/5'} mt-4 border-[#8C8C8C] rounded-[10px] border-2 bg-white dark:bg-gray-800 dark:border-gray-700"
		>
			<div class="flex p-4 flex-col w-full">
				...
				<div class="text-sm my-6 grid {isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-2">
					{#if !project?.responsibles || project.responsibles.length === 0}
						<p class="text-center {isMobile ? 'col-span-1' : 'col-span-2'} text-gray-500">
					...
					{:else}
						{#each project.responsibles as responsible}
							{#if responsible}
								<a
									href="/contacts/{responsible.id || '#'}"
									class="pl-4 border-[1.5px] border-[#B6B6B6] text-sm flex items-center gap-3 rounded-full p-1 hover:bg-blue-100 transition-colors"
								>
									<Fa icon={faUser} class="text-[16px]" style="color: #6B9AD9;" />
									<div class="flex flex-col w-full">
										<p
											class="truncate overflow-hidden whitespace-nowrap {isMobile
												? 'max-w-[120px]'
												: 'max-w-[80px]'}"
										>
											{responsible.firstName || 'Unknown'}
										</p>
										<p class="truncate overflow-hidden whitespace-nowrap max-w-[90%]">
											{responsible.lastName || 'Name'}
										</p>
									</div>
								</a>
							{/if}
		...
		<!-- Sections display -->
		<div class="mt-4">
			<DisplayerSection bind:project bind:participants {participantsNotValidated} />
		</div>

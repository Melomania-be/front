// src/routes/api/projects/[id]/management/recruitment/send-emails/+server.ts
...
export const POST: RequestHandler = async ({ params, cookies, request, fetch }) => {
	...
	try {
		...
		const res = await fetch(`${API_URL}/projects/${projectId}/management/recruitment/send-emails`, {
			method: 'POST',
			headers: {
